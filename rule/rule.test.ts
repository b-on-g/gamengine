namespace $ {
	...
	function $bog_crumb_rule_stage( spots: readonly ( readonly number[] )[] ) {
	...
	$mol_test({
		...
		'crumb under the hero is taken and hidden'() {
			const { hero, crumbs, rule } = $bog_crumb_rule_stage([ [ 0.2, 0 ], [ 5, 5 ] ])
			rule.step( 0.1 )
		...
		'all crumbs taken is a win'() {
			const { hero, rule } = $bog_crumb_rule_stage([ [ 0.2, 0 ], [ 3, 0 ] ])
			rule.step( 0.1 )
		...
		'time over without all crumbs is a loss'() {
			const { rule } = $bog_crumb_rule_stage([ [ 5, 5 ] ])
			rule.limit( 2 )
			rule.step( 1 )
			$mol_assert_equal( rule.lost(), false )
		...
		'restart brings the crumbs back'() {
			const { crumbs, rule } = $bog_crumb_rule_stage([ [ 0.2, 0 ] ])
			rule.step( 0.1 )
			$mol_assert_equal( rule.won(), true )
			rule.restart()
