namespace $ {
	...
	export const $bog_gamengine_net_probe_page = 'bog/gamengine/net/stand/-/test.html'
	...
	export const $bog_gamengine_net_probe_flags = [ '--use-angle=swiftshader' ] as const

	export const $bog_gamengine_net_probe_head_script = `
		const text = document.body.innerText
		const me = ( /me ([\\w-]{8}) /.exec( text ) || [] )[ 1 ] || ''
		const land = ( /land (\\S+)/.exec( text ) || [] )[ 1 ] || ''
		return { me, land }
	`

	export const $bog_gamengine_net_probe_move_script = `
	...
	export function $bog_gamengine_net_probe_watch_script( id: string ) {
		return `
	...
	export type $bog_gamengine_net_probe_head = {
		readonly me: string
		readonly land: string
	}

	export type $bog_gamengine_net_probe_move = {
		readonly t0: number
		readonly start: readonly [ number, number ] | null
		readonly moved: readonly [ number, number ] | null
	}

	export type $bog_gamengine_net_probe_seen = {
		readonly t1: number
		readonly before: readonly [ number, number ] | null
		readonly after: readonly [ number, number ] | null
	}

	export function $bog_gamengine_net_probe_master_alive( port = 9090 ) {
	...
	export async function $bog_gamengine_net_probe_window( bin: string, flags: readonly string[], width: number, height: number ) {
		const profile = String( $node.fs.mkdtempSync( $node.path.join( $node.os.tmpdir(), 'bog-gamengine-net-' ) ) )
	...
	export async function $bog_gamengine_net_probe_check(
		...
		if( !await $bog_gamengine_net_probe_master_alive() ) return say( $bog_gamengine_net_probe_no_master )
		...
		try {
			...
			for( let i = 0; i < 2; ++ i ) windows.push( await $bog_gamengine_net_probe_window( bin, flags, 800, 600 ) )
			const [ first, second ] = windows.map( window => window.browser )

			const page = site.uri( $bog_gamengine_net_probe_page )
			await first.open_page( `${ page }#!master=${ $bog_gamengine_net_probe_master }`, $bog_gamengine_net_probe_ready, 60000 )

			const head = await first.evaluate( $bog_gamengine_net_probe_head_script, 15000 ) as $bog_gamengine_net_probe_head
			if( !head.me ) return $mol_fail( new Error( 'первое окно не показало свой id' ) )
			if( !head.land ) return $mol_fail( new Error( 'первое окно не показало ленд комнаты' ) )
			say( `комната ${ head.land }, первый игрок ${ head.me }` )

			await second.open_page(
				`${ page }#!land=${ head.land }/master=${ $bog_gamengine_net_probe_master }`,
				$bog_gamengine_net_probe_ready,
				60000,
			)

			const seen_first = await second.until( `document.body.innerText.includes( ${ JSON.stringify( head.me ) } )`, 30000 )
			...
			for( let round = -1; round < $bog_gamengine_net_probe_rounds; ++ round ) {

				const watching = second.evaluate( $bog_gamengine_net_probe_watch_script( head.me ), 15000 )
				await $bog_probe_pause( 100 )

				const moved = await first.evaluate( $bog_gamengine_net_probe_move_script, 15000 ) as $bog_gamengine_net_probe_move
			...
			say( `задержка: ${ delays.join( ', ' ) } мс, порог ${ $bog_gamengine_net_probe_limit } мс` )

			const slow = delays.filter( delay => delay >= $bog_gamengine_net_probe_limit )
			if( slow.length ) return $mol_fail( new Error( `задержка выше порога: ${ delays.join( ', ' ) } мс` ) )

			return say( $bog_gamengine_net_probe_ok )
