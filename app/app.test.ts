namespace $ {
	...
	class $bog_crumb2_app_time_mock extends $mol_state_time {
	...
	function $bog_crumb2_app_test_run( $: $, key: string, ticks: number ) {
		$.$mol_state_time = $bog_crumb2_app_time_mock
		const app = $$.$bog_crumb2_app.make({ $ })
		$bog_crumb2_app_time_mock.stamp( 0 )
		app.Scene().step()
		app.Key().keys()[ key ]( true )
		for( let tick = 1; tick <= ticks; ++ tick ) {
			$bog_crumb2_app_time_mock.stamp( tick * 16 )
			app.Scene().step()
		}
		return app
	}
	...
	$mol_test({
		...
		'hero walks right while D is held'( $ ) {
			const app = $bog_crumb2_app_test_run( $, 'D', 20 )
			$mol_assert_ok( app.hero().pos()[ 0 ] > 1 )
		},

		'wall of the painted map stops the hero'( $ ) {
			const app = $bog_crumb2_app_test_run( $, 'D', 400 )
